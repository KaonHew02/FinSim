/**
 * ====================================================================
 * FinSim — the copy that lives in Google Drive
 * --------------------------------------------------------------------
 * localStorage stays the working store. It is instant, it works with no
 * network and no account, and FinSim is fully usable if this file never loads
...
(() => {
    ...
    const cfg = (typeof FS_DRIVE !== 'undefined') ? FS_DRIVE : null;
    ...
    async function writeFile(envelope) {
        ...
        const boundary = 'finsim-' + Math.random().toString(36).slice(2);
        ...
        try {
            ...
            if (!id) {
                return backupSay('There is nothing in Drive yet',
                    'FinSim has not written to that folder before. Press “To Drive” first, and '
    ...
    const STAMP_KEY = 'finsim.drive.lastPush';
    ...
    function showStamp() {
        ...
        if (!stamp) {
            ...
            el.title = waiting
                ...
                : 'Nothing has been sent to Drive from this browser.';
            return;
        }

        const then = new Date(stamp);
        const days = Math.floor((Date.now() - then.getTime()) / 86400000);
        el.textContent = days === 0 ? 'In Drive, today'
            : days === 1 ? 'In Drive, yesterday'
            : 'In Drive, ' + days + ' days ago';
