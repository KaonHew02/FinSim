/**
 * ====================================================================
 * FinSim — where the records actually live
 * --------------------------------------------------------------------
 ...
 *   IndexedDB once, before the forms are restored. After that `get` is a plain
 *   object lookup — synchronous and instant, so `storedRaw` and `storeRaw` in
 *   save.js kept the shape every caller already expected.
 *
 ...
 *
 * The Drive stamp deliberately stays in localStorage. It is a handful of
 * characters, it is wanted before the first paint, and it is not a record.
 *
 * FinSim and MoneyFlow are published under the same `github.io` account, which
 * makes them **one origin sharing one 5 MB bucket**. Moving both across is what
 * actually clears that, not moving either alone.
 *
...
const FSStore = (() => {
    ...
    /**
     * Every record key this app owns — what is on the forms, and the saved
     * scenarios. Preferences are not in here; neither is the Drive stamp.
     * A new store that is not listed will not persist.
     */
    const RECORD_KEYS = [
        'finsim.inputs.v1',
        'finsim.scenarios.v1',
     ...
     */
    const ASKED_KEY = 'finsim.store.persistAsked';
